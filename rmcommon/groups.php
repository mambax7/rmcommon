...
function save_group_data()
{
    ...
    if (!$xoopsSecurity->validateToken(false, true, 'CUTOKEN')) {
        $ajax->ajax_response(__('Session token expired', 'rmcommon'), 1, 0, [
                                                                        'reload' => 'reload',
                                                                    ]);
    }
    ...
    if ('' == $name) {
        $ajax->ajax_response(__('No name has been provided.', 'rmcommon'), 1, 1);
    }

    if ('' == $description) {
        $ajax->ajax_response(__('Description has not been provided.', 'rmcommon'), 1, 1);
    }

    if ($id > 0) {
        $group = new Rmcommon_Group($id);
        if ($group->isNew()) {
            $ajax->ajax_response(__('Specified group does not exists. Please try again.', 'rmcommon'), 1, 1, [
    ...
    if (!$group->save()) {
        $ajax->ajax_response(__('Errors ocurred while trying to save group data.', 'rmcommon') . "\n" . $group->errors(), 1, 1);
    }

    // Guardamos los permisos de administración
    $admin = RMHttpRequest::post('admin_modules', 'array', []);
    if (!$group->set_admin_permissions($admin)) {
        $ajax->ajax_response(__('Administrative rights could not be saved.', 'rmcommon') . "\n" . $group->errors(), 1, 1, [
                                                                                                                      'reload' => 'reload',
                                                                                                                  ]);
    }
    ...
    if (!$group->set_read_permissions($read)) {
        $ajax->ajax_response(__('Access rights could not be saved.', 'rmcommon') . "\n" . $group->errors(), 1, 1, [
                                                                                                              'reload' => 'reload',
                                                                                                          ]);
    }
    ...
    if (!$group->set_specific_permissions($admin)) {
        $ajax->ajax_response(__('Specific permissions could not be saved.', 'rmcommon') . "\n" . $group->errors(), 1, 1, [
                                                                                                                     'reload' => 'reload',
                                                                                                                 ]);
    }
    ...
    $ajax->ajax_response(__('Data saved successfully.', 'rmcommon'), 0, 1, [
                                                                       'reload' => 'reload',
                                                                   ]);
}
...
function delete_group_data()
{
    ...
    if (!$xoopsSecurity->validateToken(false, true, 'CUTOKEN')) {
        $ajax->ajax_response(__('Session token expired!', 'rmcommon'), 1, 0, ['action' => 'reload']);
    }
