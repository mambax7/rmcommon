<?php foreach ($installed_mods as $module): ?>
    <div class="module">
        <a style="background-image: url(<?php echo XOOPS_URL . '/modules/' . $module->getVar('dirname') . '/' . ('' != $module->getInfo('icon48') ? $module->getInfo('icon48') : $module->getInfo('image')); ?>);" class="image"
           href="<?php if ($module->getVar('hasadmin')): echo XOOPS_URL . '/modules/' . $module->getVar('dirname') . '/' . $module->getInfo('adminindex'); else: echo 'javascript:;'; endif; ?>"><span>&nbsp;</span></a>
